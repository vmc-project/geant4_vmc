...
/// Build user actions defined on worker threads
  ...
  if ( G4Threading::IsWorkerThread() ) {
    ...
    if ( eventAction ) {
      TG4EventAction* tg4EventAction = dynamic_cast<TG4EventAction*>(eventAction);
      if ( tg4EventAction ) {
        ...
        tg4EventAction->SetSaveRandomStatus(masterEventAction->GetSaveRandomStatus());
    ...
    if ( trackingAction ) {
      ...
      trackingAction->SetNewVerboseTrackID(fTrackingAction->GetNewVerboseTrackID());
    ...
    if ( steppingAction ) {
      steppingAction->SetLoopVerboseLevel(fSteppingAction->GetLoopVerboseLevel());
    ...
    if ( stackingAction ) {
      ...
      if ( tg4StackingAction ) {
        ...
        tg4StackingAction->SetSkipNeutrino(masterStackingAction->GetSkipNeutrino());
