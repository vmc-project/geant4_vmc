...
TG4MediumMap::TG4MediumMap() : fIdMap(), fLVMap(), fMaterialMap()
{
  /// Default constructor
}
...
TG4Medium* TG4MediumMap::AddMedium(G4int mediumID, G4bool warn)
{
  ...
  if (medium) {
    if (warn) {
      ...
      text += mediumID;
      text += " already exists.";
      TG4Globals::Warning("TG4MediumMap", "AddMedium", text);
    }
    return medium;
  }
...
void TG4MediumMap::MapMedium(G4LogicalVolume* lv, G4int mediumID)
{
  ...
  if (!medium) {
    ...
    text += mediumID;
    text += " not defined.";
    TG4Globals::Warning("TG4MediumMap", "MapMedium", text);
    return;
  }
...
void TG4MediumMap::Print() const
{
  ...
  if (fIdMap.size()) {
    G4cout << "Dump of TG4MediumMap - " << fIdMap.size()
           << " entries:" << G4endl;
    G4int counter = 0;
    std::map<G4int, TG4Medium*>::const_iterator it;
...
TG4Medium* TG4MediumMap::GetMedium(G4int mediumID, G4bool warn) const
{
  ...
  std::map<G4int, TG4Medium*>::const_iterator it = fIdMap.find(mediumID);
  ...
  if (it == fIdMap.end()) {
    if (warn) {
      ...
      text += mediumID;
      text += " not defined.";
      TG4Globals::Warning("TG4MediumMap", "GetMedium", text);
    }
    return 0;
  }
...
TG4Medium* TG4MediumMap::GetMedium(const G4String& name, G4bool warn) const
{
  ...
  if (warn) {
    TG4Globals::Warning("TG4MediumMap", "GetMedium",
...
TG4Medium* TG4MediumMap::GetMedium(G4LogicalVolume* lv, G4bool warn) const
{
  ...
  std::map<G4LogicalVolume*, TG4Medium*>::const_iterator it = fLVMap.find(lv);
...
//_____________________________________________________________________________
TG4Medium* TG4MediumMap::GetMedium(
  const G4Material* material, G4bool warn) const
{
  ...
  if (it == fMaterialMap.end()) {
    if (warn) {
      TG4Globals::Warning("TG4MediumMap", "GetMedium",
        "Medium for material " + TString(material->GetName()) +
          " is not found.");
    }
    return 0;
  }
