...
/// \file TMCRootManagerMT.cxx
/// \brief Implementation of the TMCRootManagerMT class
///
...
TMCRootManagerMT::TMCRootManagerMT(
  const char* projectName, TVirtualMCRootManager::FileMode fileMode)
  : TVirtualMCRootManager(), fId(0), fRootManager(0)
{
  ...
  if (fgDebug)
    printf("Going to new TMCRootManagerImpl in %d  %p \n", fId, this);
...
//_____________________________________________________________________________
void TMCRootManagerMT::Register(
...
void TMCRootManagerMT::WriteAndClose()
{
  ...
  if (fgDebug)
    printf("Going to lock for WriteAndClose in %d  %p \n", fId, this);
  TMCAutoLock lk(&writeMutex);
  ...
  lk.unlock();
  if (fgDebug)
    printf("Released lock for WriteAndClose in %d  %p \n", fId, this);
}
