...
class Ex03dParticle
{
 public:
  ...
  Ex03dParticle(Int_t pdg, Int_t status, Int_t mother1, Int_t mother2,
  ...
  Double_t PhiX() const
  {
    return TMath::Pi() + TMath::ATan2(-fPz, -fPy);
  } // note that PhiX() returns an angle between 0 and 2pi
  Double_t PhiY() const
  {
    return TMath::Pi() + TMath::ATan2(-fPx, -fPz);
  ...
  {
    return (fPx == 0) ? TMath::PiOver2() : TMath::ACos(fPx / P());
  }
  Double_t ThetaY() const
  {
    return (fPy == 0) ? TMath::PiOver2() : TMath::ACos(fPy / P());
  }
  ...
  Double_t Theta(
    const Ex03dParticle& p) // Returns the angle between momenta of particles
  {
    Double_t v = P() * p.P();
    if (v == 0)
      v = 1;
    else
      v = (fPx * p.Px() + fPy * p.Py() + fPz * p.Pz()) / v;
    if (v > 1)
      v = 1;
    else if (v < -1)
      v = -1; // just a precaution
    return TMath::ACos(v);
  }
  ...
  {
    Double_t pmom = P();
    if (pmom != TMath::Abs(fPz))
      return 0.5 * TMath::Log((pmom + fPz) / (pmom - fPz));
    else
      return 1.e30;
  }
  Double_t Y() const
