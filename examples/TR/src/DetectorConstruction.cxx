...
void DetectorConstruction::ConstructGeometry()
{
  ...
  TGeoMixture* matMylar
    = new TGeoMixture("Mylar", 3, density = 1.39);
  ...
  // Carbon dioxide, STP
  TGeoMixture* matCO2
    = new TGeoMixture("CO2", 2, density = 1.977e-06);
  matCO2->AddElement(elC,1);
  matCO2->AddElement(elO,2);

  // 85% Xe + 15% CO2, STP
  TGeoMixture* matXe15CO2
    = new TGeoMixture("Xe15CO2", 2, density=4.9e-03);
  ...
  TGeoMixture* matRadiator
    = new TGeoMixture("radiatorMat", 2, totDensity);
  ...
  Double_t electrodeZ = gapZ + gapThick/2. + electrodeThick/2. + 0.01*mm;
  Double_t absorberZ = electrodeZ + electrodeThick/2. + absorberThickness/2. + 0.01*mm;

  logicWorld->AddNode(logicAbsorber, 1, new TGeoTranslation(0., 0., absorberZ));
  ...
  cout << "fRadiatorMat = " << radiatorMedium->GetMaterial()->GetName() << endl;
